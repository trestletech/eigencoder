...
listToDensity <- function(lst, title="", xlab="x", ...){
  dens <- lapply(lst, density)
  df <- data.frame(
    x = unlist(lapply(dens, "[[", "x")),
    y = unlist(lapply(dens, "[[", "y")),
    lang = rep(names(dens), each = length(dens[[1]]$x))
  )
  
  plot_ly(df, x=x, y=y, color=lang) %>% 
    layout(title=title, xaxis=list(title=xlab), yaxis=list(title="Ratio"), ...)
}
...
listToBar <- function(lst, fun, title="", ylab="Average"){
  ...
  plot_ly(df, x=language, y=avg, type="bar") %>% 
    layout(title=title, yaxis=list(title=ylab))
}
